...
export default function CreateDog() {
    ...
    const handleSubmit = (e)=>{
     ...
     if(!errors.name  && !errors.life && !errors.height_min && !errors.height_max && !errors.weight_max && !errors.weight_max && !errors.temperaments){
...
<form className='form' onSubmit={(e)=>handleSubmit(e)} >
    ...
    {errors.name &&(
           <h4 className='error'>{errors.name}</h4>
         )}
    ...
    <select onChange={(e)=>handleSelect(e)} >
{
 ...
 <button className='bot' type='submit'>Create Dog</button>

 </form>

 <div className='card1'>
<h1>Look how it's doing</h1>
<Card image={input.image?input.image:'https://i.pinimg.com/originals/28/1a/af/281aaf73c5b37d8be3b0ed20caade1c9.jpg'} name={input.name} temperaments={input.temperaments.toString()} weight_min={input.weight_min} weight_max={input.weight_max}  ></Card>
</div>
...
<div className='tempCont' >
 ...
 {input.temperaments.map(el=>
  <div className="delete">
   <h4>{el}</h4>
   <button className='but'onClick={()=>handleDelete(el)}>x</button>
  </div>)}
